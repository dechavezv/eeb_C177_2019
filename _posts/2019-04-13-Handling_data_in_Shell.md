...
Login into Hoffman. Then, copy the directory `Example_cat/` located at `classdata/Labs/Lab3/` 
...
Although this is information is somewhere in the file 
...
Let's create a directory called `Individual_genes`  
then move the files that start eith the name `Gene_...txt` to the directory you just created.  
**IMPORTANT**: DO NOT MOVE the file `All_genes.txt`    
...
~~~
[c177-t0@login2 ~]$ {: ..bash-language} less -F All_Genes.txt 
~~~
{:  .language-bash}
...
`sed` is usefule to create and edit tables. Usually we used it to 
replace especial characters with tabs as a delimiter.
...
~~~
What happened ??? everything got replace with tabs. The reason is that 
`.` is a wildcard that indicates any character if you want to replace an actuall dot (`.`), you 
write '\.'
...
We can also use a combination of other simple commands to sort and chop tables.  

~~~
[c177-t0@login2 ~]$ echo "Ensmebl_ID%Gene_Symbol%Statistic..Pvalue" > Table_sed_sort_chop.txt
[c177-t0@login2 ~]$ echo "ENSCAF70%PHLPP1%14.962838..0.000109649" >> Table_sed_sort_chop.txt
[c177-t0@login2 ~]$ echo "ENSCAF59%CAMK2D%14.77195..0.000121327" >> Table_sed_sort_chop.txt
[c177-t0@login2 ~]$ echo "ENSCAF31%B4GALT7%14.093764..0.000173919" >> Table_sed_sort_chop.txt 
[c177-t0@login2 ~]$ echo "ENSCAF70%PHLPP1%14.962838..0.000109649" >> Table_sed_sort_chop.txt
[c177-t0@login2 ~]$ echo "ENSCAF59%CAMK2D%14.77195..0.000121327" >> Table_sed_sort_chop.txt
[c177-t0@login2 ~]$ echo "ENSCAF31%B4GALT7%14.093764..0.000173919" >> Table_sed_sort_chop.txt
~~~ 

As you now, the following will create a table  
~~~
...
ENSCAF31	B4GALT7	14.093764	0.000173919
~~~
**Note**: `-n` means that you want to sort number and `-k` means that you want to based your 
...
Finally if you are interested only in genes symbols and their pvalues 
...
### First challenge  
You will work with the file `All_Genes.txt`. Rigth now the infromation looks somethin like this: `GeneID10DCH..sequenced..CA@19.##0#3##.10%UCLA..replicateB`.  
You will have to fixed the information in `All_Genes.txt` to make it a tab-delimitated table. Then,
you will have to sort them and delete duplicated fields. The file should have 9 colums with the following infromation:  
- 1st_column Gene_ID.    
...
These is list of steps that you have to do for this challenge:  

1. Change scpecial symbols `%`, `..`, `@` to tabs.  
2. Sort the file acording to values of the 6th column (Day of sequencing).  
3. Save only the 1st and 6th column into a file called `Sort_All_Genes_<YOUR_NAME>.txt`.  
4. Open that folder and answer the following question:  

Charles was working with African wild dog DNA and just realized that the gene sequenced 
...
that is loaced at `classdata/Labs/Lab3`. Let’s see what the files we just copied look like.  
...
if we merge the files into a single file that is tab-delimitated. Go ahead and merge all files and replace ',' 
for tabs with `sed`. Save the file as `transcriptome.gtf`. You should get something like this.  
...
Copy the file `Example.bed`. This is a file with **31,558 lines**!!! Good luck trying excel. In this challenge 
...
- The 1st tell us in what chromosome the genes is located.  
- The 2nd colum is the postion of the gene in the chromosome.    
- The 3rd colum is the unique ID of the transcript; A single gene can.    
...
1. Create a new file containing only the names of genes. Save the file with the name `Only_genes_<YOURNAME>.txt 
at the location  `classdata/Labs/Lab3/Second_challenge`.  
2. What is the number of transcripts for the gene 'FGFR3'.  
2. Number of genes present in chr01.  
3. Save a new file name Only_genes_<YOURNAME>.txt containing only the names of genes. There should not be duplicated names. 
Coy the file to `classdata/Labs/Lab3/Second_challenge`.  
